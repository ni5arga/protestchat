...
- **Groups with an unverified member showed the same green "Private" treatment as a fully verified direct chat (found 22 Jul 2026, security audit).** `describeConversation()` hardcoded `tone: 'ok'` for every group regardless of member verification state, so a group is the mode with the *most* MITM exposure (N independent safety-number checks, one bad key ruins the room) and the *least* warning about it — a stressed user who added an impersonator at introduction was told, in green, that only their crew could read it. Group tone is now derived from member verification the same way direct-chat tone already was: any unverified member (or a member with no contact record at all) turns the mode notice, the home-screen tag, and the eyebrow amber, naming how many members are unverified, and re-derives whenever verification state changes.
